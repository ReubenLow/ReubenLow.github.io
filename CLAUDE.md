...
**Media in content** goes through includes rather than raw HTML: an `include` of `figure.liquid`,
`video.liquid`, or `audio.liquid` — these handle responsive WebP variants, lazy loading, and zoom.
Images referenced from `assets/img/` get 480/800/1400px WebP siblings generated by
`jekyll-imagemagick` at build time.

Do not write a literal Liquid tag in this file. It is listed in `exclude:` in `_config.yml` so Jekyll
skips it, but if that ever changes, an unescaped tag here is executed at build time and breaks the
deploy (this happened once: a `figure.liquid` include tag in this file was published to `gh-pages`,
where GitHub's own Jekyll pass could not resolve it, and the site froze on the last good build).
